...
public class EnemyFieldOfView : MonoBehaviour
{
    ...
    [Header("View Config")]
    [Range(0f, 360f)]
    [SerializeField] public float m_horizontalViewAngle = 0f; // 시야각
    [SerializeField] public float m_viewRadius = 1f; // 시야 범위
    [Range(-180f, 180f)]
    ...
    public Collider2D[] FindViewTargets()
    {
        ...
        foreach (Collider2D hitedTarget in hitedTargets)
        {
            ...
            if (angle <= m_horizontalViewHalfAngle)
            {
                ...
                RaycastHit2D rayHitedTarget2 = Physics2D.Raycast(originPos, dir, m_viewRadius, m_viewTargetMask);
                if (rayHitedTarget)
                {

                    if (rayHitedTarget2 && Vector2.Distance(transform.position, rayHitedTarget.transform.position) < Vector2.Distance(transform.position, rayHitedTarget2.transform.position))
                    {
                       enemy.state = Enemy.enemyState.following;
                    }
